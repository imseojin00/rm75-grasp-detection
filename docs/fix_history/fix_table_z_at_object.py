...
old = '''    def get_table_z_in_camera(self):
...
new = '''    def get_table_z_in_camera(self, object_point_cam=None):
        """
        ...
        if object_point_cam is None:
            # 카메라 바로 아래 (기존 방식, 하위호환용)
            table_point_base = np.array([t.x, t.y, TABLE_Z_BASE])
        else:
            # 물체의 실제 위치(base_link)를 구하고, 그 x,y에서 테이블 높이 사용
